...
void j1Map::PropagateBFS()
{
	...
	iPoint neighbourElement[4];
	if (frontier.GetLast() != nullptr) {
		iPoint last = frontier.GetLast()->data;
		frontier.Pop(last);
		neighbourElement[0] = {last.x + 1, last.y};
		neighbourElement[1] = {last.x - 1, last.y};
		neighbourElement[2] = {last.x, last.y + 1};
		neighbourElement[3] = {last.x, last.y - 1};
	}
	// TODO 2: For each neighbor, if not visited, add it
	// to the frontier queue and visited list
	for (int i = 0; i < 4; i++) {
		if (visited.find(neighbourElement[i]) == -1) {
			frontier.Push(neighbourElement[i]);
			visited.add(neighbourElement[i]);
		}
	}
}
